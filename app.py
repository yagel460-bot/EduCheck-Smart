...
LANG_DICT = {
    "עברית": {
        ...
        "student_reg": "📝 רישום תלמיד חדש (קבוע)",
        "save_btn": "שמור במאגר", "select_student": "👤 בחר תלמיד:",
        "exam_type": "📝 סוג מבחן:", "rubric_label": "🎯 מחוון",
        "types": ["פתוח", "אמריקאי", "השלמה", "נכון/לא נכון", "מתמטיקה"],
        "btn_check": "התחל בדיקה ושמירה 🚀", "scan_msg": "מנתח ושומר..."
    },
    "English": {
...
# --- אפליקציה ראשית ---
selected_lang = st.sidebar.selectbox("🌐 Language", ["עברית", "English"])
L = LANG_DICT[selected_lang]
apply_style(L["dir"], L["align"])

# יצירת מאגר נתונים פיזי
...
with st.sidebar.expander(L["student_reg"]):
    ...
    files = st.file_uploader("3 דגימות כתב יד", type=['png', 'jpg', 'jpeg'], accept_multiple_files=True)
...
if not students:
    st.info("אנא רשום תלמיד בסיידבר.")
else:
    ...
    with c2:
        exam_f = st.file_uploader("העלאת מבחן (Img/PDF/Word)", type=['png', 'jpg', 'jpeg', 'pdf', 'docx'])
    with c3:
    ...
    if st.button(L["btn_check"]):
        if exam_f and rubric:
            with st.status(L["scan_msg"]):
                ...
                # חילוץ טקסט אם זה PDF/Word
                content = [prompt] + samples
                if exam_f.type.startswith("image"):
                    content.append(Image.open(exam_f))
                else:
                    content.append("Text content extracted from document.") # לוגיקת החילוץ כאן
